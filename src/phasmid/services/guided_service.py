...
def get_workflows() -> list[GuidedWorkflow]:
    return [
        ...
        GuidedWorkflow(
            ...
            steps=[
                GuidedStep(1, "Select a Vessel file.",
                    "Choose any Vessel from the list."),
                GuidedStep(2, "Run inspection.",
                    "The inspection service reads the file without decrypting it."),
                GuidedStep(3, "Review inspection output.",
                    "Expected output: no recognized header detected, "
                    "no obvious magic bytes detected, high / random-like entropy."),
                GuidedStep(4, "Note: inspection does not confirm deniability.",
