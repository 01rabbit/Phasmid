...
| `src/phasmid/local_state_crypto.py` | AES-GCM (encrypt/decrypt) | `cryptography` `AESGCM` | Key: 256-bit; Nonce: 96-bit random (prepended to blob); Tag: 128-bit; AAD: caller-supplied | `crypto_params.AESGCM_*` | **Low** | Used for all local state blobs (ORB store, role store, face lock). |
