...
- When UI face lock is enabled, camera preview and object-match state are withheld until the UI is unlocked. First-time WebUI face enrollment requires explicit setup mode with `PHANTASM_UI_FACE_ENROLL=1` or a short-lived enrollment request created by the CLI reset flow.
...
- Reload `/ui-lock` after `python3 main.py reset-face-lock` to consume the short-lived enrollment request. The request expires and is removed after successful enrollment.
